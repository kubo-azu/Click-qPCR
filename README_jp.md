...
-   **診断タブ:**
    ...
    -   このタブは、アプリの自己診断機能を提供します。`Run Diagnostics`ボタンを押すと，アプリに内蔵されたサンプルデータを用いて，以下の3つの主要な機能が正しく動作しているかを自動でテストします。

        1. サンプルデータの読み込み: データが問題なくロードできるか
        2. ΔCq解析の検証: ΔCq値の計算とt検定が，期待される統計的有意差を正しく検出できるか
        3. ΔΔCq解析の検証: ΔΔCq値（Fold Change）の計算とt検定が，同様に期待される結果を正しく示せるか
